...
parser.add_argument("--scoring_model_name", type=str, default="tiiuae/falcon-7b")  # 用于打分的模型（Score model）
parser.add_argument("--reference_model_name", type=str, default=None)   # 参考模型（Ref model），若为None则与score model相同
parser.add_argument("--cache_dir", type=str, default="./model/")  # HF权重缓存目录（本地优先）
parser.add_argument("--discrepancy_analytic", type=bool, default=False)  # 是否使用解析式差异（analytic）而非采样近似
...
parser.add_argument('--save_path', type=str, default='./results/FastDetectGPT')  # 结果保存目录
parser.add_argument('--save_file', type=str, default='eval_fast_detect_gpt.json')  # 结果保存文件名
...
def get_samples(logits, labels):
    """
        从参考分布（logits）中进行类别采样，作为 \tilde{x} 的近似样本。
        形状约定：
        - logits: [B, T, V]（batch, 序列长, 词表大小）
        返回：
        - samples: [B, T, nsamples]（每个时间步采样 nsamples 次）
        说明：
        - 这里 nsamples=10000，可能较大，内存与时间开销可观；必要时可调小以加速。
    """
    nsamples = 1024
...
def get_likelihood(logits, labels, attention_mask=None):
     # labels: [B, T] 或 [B, T, nsamples]
    labels = labels.unsqueeze(-1) if labels.ndim == logits.ndim - 1 else labels
    lprobs = torch.log_softmax(logits, dim=-1)
    log_likelihood = lprobs.gather(dim=-1, index=labels).squeeze(-1)  # [B,T] 或 [B,T,S]

    if attention_mask is not None:
        mask = attention_mask[:, 1:].float()                 # [B, T]
        mask = mask[:, :log_likelihood.shape[1]]             # 对齐到实际 T

        if log_likelihood.ndim == 3:                         # [B, T, S]
            mask = mask.unsqueeze(-1)                        # [B, T, 1] 以便广播到样本维
            # 按时间维(T, 即 dim=1)加权汇总，保留样本维 S -> [B, S]
            log_likelihood = (log_likelihood * mask).sum(dim=1) / (mask.sum(dim=1) + 1e-8)
        else:                                                # [B, T]
            # 按时间维(T, 即 dim=1)加权汇总，得到 [B]
            log_likelihood = (log_likelihood * mask).sum(dim=1) / (mask.sum(dim=1) + 1e-8)
    else:
        if log_likelihood.ndim == 3:                         # [B, T, S]
            log_likelihood = log_likelihood.mean(dim=1)      # 沿时间维取均值 -> [B, S]
        else:                                                # [B, T]
            log_likelihood = log_likelihood.mean(dim=1)      # -> [B]
